...
在社交媒体的多主体共存环境中，评论能否引发后续回应是衡量其传播影响力的关键信号——回复行为要求受众在阅读并理解原始评论的基础上形成自身观点并组织语言进行表达，涉及深层次的互动参与。面向这一场景，系统需要在评论级别量化不同主体的传播影响力分布，为每条评论输出传播价值评分，以区分人类用户与AI代理在内容吸引力上的差异。在系统功能定位上，本章对应评论级离线评估模块，需要在采集完成后基于同一输入条件对全量评论进行统一评分，服务于评论排序、价值分层和主体间统计比较，而不承担已发布评论的实时热度追踪或动态更新。因此，模型输入限定为发布时刻可获得的静态信息，模型输出用于刻画固定观测窗口下的传播价值评分，而不是对发布后已知互动反馈的复述。本章针对这一需求，提出Group-aware Fusion TabTransformer模型（其中"组"指四类异构特征组，"感知"指模型能自适应识别各特征组对不同样本的重要性差异），通过将主体标识（agent\_type）作为显式特征组（G4）引入并以门控融合机制自适应调整各特征组的样本级权重，实现对评论传播价值的准确评估。

将评论传播价值评估建模为预测任务，首先需要确定衡量传播价值的具体标准。本章选择评论在时间窗口$T$内是否获得至少一条直接回复作为判定标准。在社交媒体平台的多种用户互动行为中，浏览和点赞属于低成本、低参与度的操作，用户无需对内容进行深入理解即可完成，更多反映信息的曝光度和表层吸引力；相比之下，回复行为涉及信息理解、立场判断与内容生成等深层次互动参与，能够更真实地反映用户对评论内容的关注投入程度，是衡量信息是否引发实质性传播互动的有效信号。

为实现不同评论之间传播价值的公平比较，需要引入统一的时间窗口$T$对回复观测进行约束。不同评论的发布时间各异，若不设定统一的观测窗口，"是否被回复"这一指标将受到观测时长的影响，导致不同评论之间的比较缺乏公平性。时间窗口$T$的选取需要在两个因素之间权衡：一方面是正例覆盖度，即在$T$时间内真正被回复的评论有多少能被窗口捕获到（$T$过短会遗漏），$T$过短导致大量实际会被回复的样本被错误标记为负例；另一方面是数据利用率，$T$过长导致大量评论的观测时间不足$T$（即右删失样本——观测窗口结束时事件尚未发生，无法判断最终是否会被回复），这些样本标签不确定，无法使用。本文选用$T=12\text{h}$作为时间窗口参数，具体的数据依据将在\ref{sec:dataset}节基于数据集分布进行详细论证。
...
现有公开社交媒体数据集普遍缺乏对内容发布主体类型（人类用户或AI代理）的标注信息，无法直接支持多主体传播行为的对比研究。因此，本文基于某社交平台构建了包含人类用户与AI代理A评论的自建数据集。

数据采集采用三阶段流水线方式进行：首先从公开趋势话题页面获取目标地区24小时内持续时间最长的前5个热门趋势话题作为采集种子，选择持续时间较长的话题旨在确保话题下已积累足够数量的评论；随后基于话题关键词在某社交平台检索每个话题下热度最高的前5条根消息；最后对每条根消息递归获取其完整的评论树结构，包括直接评论及多层嵌套的子评论，若某条消息本身为其他消息的评论，则回溯至最顶层根消息后进行完整采集。采集过程中同步记录trending\_zone（话题来源地区）和search\_keyword（检索关键词）等元数据字段。选择从热门趋势入手进行采集，主要考量是热门话题的时效性能够确保采集窗口覆盖AI代理功能上线后的评论数据，从而保证数据集中包含足够的AI代理样本。

考虑到社交媒体数据包含平台账号、评论文本和互动关系等信息，本文仅将公开评论数据用于科研分析和模型验证，并在处理与论文展示环节控制个人可识别信息的使用范围。原始平台用户ID和用户名仅用于评论树结构关联、主体类型识别和去重，不作为传播价值评估模型或传播延续预测模型的输入特征；在统计分析、实验结果和系统展示说明中，重点呈现聚合指标、主体类型、评论结构和模型评分，不对单个用户进行画像或结论性评价。

原始采集数据经comment\_id去重后包含255,754条唯一评论。对其进行质量过滤，剔除crawl\_time缺失（64,631条）和comment\_text为空（8,600条）的样本，共移除73,231条（28.63\%），保留\textbf{182,523}条。评论的回复标签基于实际采集到的评论树父子关系判定（即在评论树中是否存在直接子回复），而非平台返回的reply\_count元数据字段，基于评论树结构判定的有回复评论为54,359条。

\subsection{时间窗口选取与最终数据集}

$T$的选择需要同时平衡两个指标：正例覆盖度（即被回复的评论中有多少比例的首次回复落在$T$以内）$P(\Delta \leq T \mid \text{replied})$，以及删失率（即因观测时间不足$T$而无法使用的样本比例）$P(A < T)$。对全量数据中所有存在直接子回复的评论（$n=54{,}359$），首次回复延迟$\Delta$的中位数仅16.75分钟；$T=12\text{h}$时覆盖96.47\%，$T=24\text{h}$时仅额外提升2.68个百分点。在删失率方面，$T=12\text{h}$时删失率为12.90\%（87.10\%样本可用），而$T=24\text{h}$时骤升至50.43\%。

综合上述分析（如图\ref{fig:t_selection}所示），$T=12\text{h}$为帕累托最优点：正例覆盖度96.47\%，删失率仅12.90\%，且符合社交媒体研究中12--24小时短期传播窗口的领域惯例\cite{pfeffer2023halflife}。基于此选定$T=12\text{h}$后，根据4.1节提出的无右删失约束（$A \geq T$），剔除随访时间不足12小时的样本23,551条（12.90\%）。最终数据集包含\textbf{158,972}条评论，其中Human 155,529条（97.83\%）、Agent 3,443条（2.17\%）；整体正例率22.79\%，但两类主体的正例率差异显著：Human为22.10\%，Agent高达54.26\%。

此外，本数据集的覆盖范围具有明确边界。数据来源集中于某社交平台，AI代理主体为AI代理A，且Human与Agent样本数量存在不平衡（97.83\%/2.17\%）。这一设置与本文关注的单一平台人类用户和AI代理共存场景相匹配，也有助于在相对可控的主体设定下分析多主体传播差异；但实验结论仍应主要解释为该平台和该AI主体条件下的结果，其跨平台、多AI代理类型以及更均衡样本分布下的泛化能力仍需后续数据验证。

\vspace{0.5\baselineskip}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{G3-4/T_selection_dual_axis.png}
  \caption{正例覆盖度与删失率随T变化的双轴对比图}
...
从系统功能定位看，本文实现的评估系统面向评论传播价值评估、价值排序和分层展示，输入为采集与特征工程阶段已经固化的评论文本、发布环境、情感倾向和主体标识等信息，输出为可用于统一比较的传播价值评分。系统不承担已发布评论的实时热度追踪、流式监测或预测结果滚动更新功能，也未设计持续拉取发布后互动反馈并重算评分的模块。因此，本章在模型实现上统一采用发布时刻静态特征作为输入，并将时间窗口$T$内是否收到直接回复作为监督标签；发布后的浏览、点赞、转发或早期回复等反馈不作为输入特征，以保持训练阶段与系统推理阶段的输入一致。

图\ref{fig:pipeline}展示了本任务的完整物理流程，涵盖从某社交平台多主体数据采集、数据清洗、特征工程到模型推理的端到端过程。

\vspace{0.5\baselineskip}
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{G3-1-latex/fig_3x_pipeline.pdf}
  \caption{评论传播价值评估任务全流程示意图}
  \label{fig:pipeline}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{G3-2/label_timeline.png}
  \caption{时间窗口与标签定义示意图}
...
\begin{figure}[H]
  ...
  \caption{Group-aware Fusion TabTransformer模型结构图}
  \label{fig:model_arch}
\end{figure}

模型的处理流程可概括为以下四个阶段：

\textbf{（1）组投影阶段}：将四组异构特征（G1发布环境、G2文本语义、G3情感倾向、G4主体标识）分别通过独立的投影器映射到统一的$d_{\text{model}}$维表示空间，生成$n$个组token $\{\mathbf{e}_1, \mathbf{e}_2, \ldots, \mathbf{e}_n\}$。该阶段消除了不同特征组之间的维度差异，为后续的融合与交互奠定基础。
...
\textbf{（3）Transformer编码阶段}：将所有组token与融合token拼接后输入Transformer编码器进行深层特征交互。拼接序列记为$[\mathbf{e}_1; \ldots; \mathbf{e}_n; \mathbf{e}_{\text{fused}}]$。通过多头自注意力机制，模型能够捕捉不同特征组之间的复杂依赖关系。
...
发布环境特征捕捉评论发布时的时空背景信息，包含三个类别型子特征：地理区域（trending\_zone），即评论所属热点话题的地理区域，包含美国、加拿大、印度三个类别，采用独热编码表示，维度为3；发布小时（hour\_of\_day），从评论发布时间戳中提取的UTC小时数（0--23），采用独热编码表示，维度为24；发布星期（day\_of\_week），从评论发布时间戳中提取的星期几（0=周一，6=周日），采用独热编码表示，维度为7。G1特征组总维度为34维（$3+24+7$），全部采用独热编码形式，反映了评论发布的外部环境因素，这些因素独立于评论内容本身，但可能影响用户的在线活跃度和互动倾向。

\textbf{（2）G2文本语义特征}

文本语义特征通过预训练语言模型提取评论文本的深层语义表示。以评论原始文本为输入，使用BAAI/\allowbreak bge-base-en-v1.5模型进行编码，该模型基于BERT架构、专为文本嵌入任务优化，输出768维稠密向量。该特征组捕捉评论的语义内容、表达方式和话题相关性，是判断评论传播价值的核心信号之一。

\textbf{（3）G3情感特征}

情感特征量化评论文本的情感倾向。以评论原始文本为输入，使用cardiffnlp/\allowbreak twitter-roberta-base-sentiment-latest模型进行分析，该模型基于RoBERTa架构、在Twitter数据上微调\cite{barbieri2020tweeteval}，适用于社交媒体文本的情感分析，输出三维概率向量$(p_{\text{pos}}, p_{\text{neu}}, p_{\text{neg}})$，分别表示正面、中性、负面情感的概率，满足$p_{\text{pos}} + p_{\text{neu}} + p_{\text{neg}} = 1$。情感特征反映评论的情绪色彩，已有研究表明情感极性与社交媒体内容的传播效果存在显著关联\cite{stieglitz2013emotions,tsugawa2017sentiment}。

\textbf{（4）G4主体标识特征}

主体标识特征区分评论发布者的类型，取值为二元类别变量，其中0表示人类用户（Human），1表示AI代理（Agent，本数据集中对应AI代理A）。判定方式为通过平台账号的user\_id与username双重匹配：目标AI代理账号具有唯一且固定的平台用户ID，采集脚本对每条评论的发布者信息进行匹配，同时满足user\_id和username条件的评论标记为agent\_type=1，其余标记为agent\_type=0。编码方式采用可学习的嵌入向量（Embedding），将离散标识映射到$d_{\text{model}}$维连续空间。该特征是本研究的核心创新点之一，使模型能够显式感知并区分不同主体类型的传播行为模式差异。
...
通过$L$层编码器的堆叠，模型能够捕捉不同特征组之间的高阶依赖关系。例如，文本语义特征（G2）与主体标识特征（G4）的交互可能揭示人类用户与AI代理在表达方式上的差异模式。设编码器输出为$\mathbf{H} = [\mathbf{h}_1; \ldots; \mathbf{h}_n; \mathbf{h}_{\text{fused}}] \in \mathbb{R}^{(n+1) \times d_{\text{model}}}$，其中$\mathbf{h}_1,\ldots,\mathbf{h}_n$分别对应各特征组token的编码结果，$\mathbf{h}_{\text{fused}}$为融合token的编码结果。
...
本章实验使用\ref{sec:dataset}节构建的158,972条评论数据集。采用train/val/test三段式划分，用于模型训练、超参数选择与最终评估。为避免同一评论树的评论在训练集与测试集中同时出现造成语境信息泄露，划分以root\_tweet\_id（评论树根推文）为分组单位，确保同一评论树的所有评论仅出现在同一个子集中。数据按70/15/15比例划分，训练集104,384条、验证集25,174条、测试集29,414条，各子集的正例比例（22.74\%--22.93\%）与human/agent比例（约97.8\%/2.2\%）均与全量分布保持一致。每组实验使用3个随机种子（seed=1/2/3），结果取平均报告。
...
所有基线模型与本文模型使用相同的G1--G4特征组作为输入。LR和XGBoost将所有特征拼接为平坦向量直接输入；TabTransformer和AutoInt对类别型特征（G1发布环境中的子特征、G4主体标识）采用嵌入编码，连续型特征（G2文本语义、G3情感倾向）按其各自方式处理。上述四个基线构成从线性模型到非线性集成再到深度注意力的递进对比体系。
...
分主体结果揭示了两个关键现象。第一，所有模型在Agent子集上的表现均远优于Human子集：以AUC-PR为例，Agent子集（0.588--0.621）显著高于Human子集（0.389--0.400）；Recall@P=0.5差异更为悬殊（如LR：0.836 vs 0.167）。这一差异的根本原因在于数据分布：Agent评论的正例率为54.26\%，远高于Human的22.10\%，较高的先验概率使得模型在Agent子集上更容易实现高性能。第二，Group-aware Fusion TabTransformer在Agent子集上全面领先：A-AUC-PR达到0.621，较次优的LR（0.605）提升2.6\%；A-Recall@P=0.5取得最优（0.853），说明组感知结构通过显式引入主体标识特征（G4）并自适应调整组权重，能够更好地捕捉Agent群体的传播行为模式。值得注意的是，深度注意力模型（TabTransformer: 0.797，AutoInt: 0.708）在A-Recall@P=0.5上反而不及传统方法（XGB: 0.843），表明单纯引入注意力机制而不进行组级建模，并不能自动适配多主体场景。由此可见，将主体类型从普通离散特征提升为独立特征组，有助于在评论级传播价值评估中显式保留人类用户与AI代理的行为差异。
...
通过表\ref{tab:ablation}的消融实验结果可以看出，门控融合机制与主体标识特征均对模型性能具有正向贡献，且二者的作用维度有所不同。将门控权重替换为均匀权重（Mean Fusion）后，整体AUC-PR仅从0.422微降至0.421，但在分主体视角下差异更为显著：A-Recall@P=0.8从0.100下降至0.078（降幅22\%），H-Recall@P=0.8从0.024降至0.017（降幅29\%），而A-AUC-PR基本持平（0.621 vs 0.619）。这表明门控融合机制的价值主要体现在高精度筛选场景下——通过样本级自适应权重分配，提升模型在不同主体子群体上的精细化识别能力。相比固定平均融合，门控融合能够根据样本差异调整发布环境、文本语义、情感倾向和主体标识的相对贡献，这与多主体评论样本内部异质性较强的任务特点相匹配。

去除G4后，整体AUC-PR下降至0.418，Recall@P=0.5从0.215降至0.196；分主体指标下降更明显：A-AUC-PR从0.621降至0.608，A-Recall@P=0.5从0.853降至0.691，A-Recall@P=0.8从0.100降至0.035，而human子集基本持平。这说明G4是捕捉agent传播行为模式的核心信号，缺少主体标识会显著削弱agent高传播价值样本识别能力。结合基线对比和消融结果，本章方法的主要贡献在于面向人类用户与AI代理共存场景构建主体感知的组级融合机制，而非单纯增加通用模型复杂度。

\section{本章小结}

本章提出Group-aware Fusion TabTransformer模型，通过将主体标识作为显式特征组并引入门控融合机制，实现了评论级别的传播价值量化评估。在某社交平台158,972条评论的多主体数据集上，模型在整体与分主体指标上均优于四个基线；与TabTransformer、AutoInt等注意力模型的对比说明，组级建模比单纯引入通用注意力结构更适合本文的多主体场景。消融实验进一步验证了主体标识特征与门控融合机制的有效性，其中G4特征组对Agent高传播价值评论识别贡献尤为明显。

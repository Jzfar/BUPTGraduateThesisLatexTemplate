...
在社交媒体的多主体共存环境中，评论能否引发后续回应是衡量其传播影响力的关键信号——回复行为要求受众在阅读并理解原始评论的基础上形成自身观点并组织语言进行表达，涉及深层次的互动参与。面向这一场景，系统需要在评论级别量化不同主体的传播影响力分布，为每条评论输出传播价值评分，以区分人类用户与AI代理在内容吸引力上的差异。该需求要求模型仅使用评论发布时刻的静态特征进行预测，不依赖发布后的传播反馈。由此，模型输出的传播价值评分反映的是评论基于发布时刻特征的内禀传播潜力——即评论内容本身吸引回复的能力——而非对已知传播结果的复述。在系统的评估分析场景中，模型对数据集中全量评论统一评分，使得按主体类型分组聚合后能够从统计分布层面揭示两类主体在传播潜力上的系统性差异。本章针对这一需求，提出Group-aware Fusion TabTransformer模型（其中"组"指四类异构特征组，"感知"指模型能自适应识别各特征组对不同样本的重要性差异），通过将主体标识（agent\_type）作为显式特征组（G4）引入并以门控融合机制自适应调整各特征组的样本级权重，实现对评论传播价值的准确评估。

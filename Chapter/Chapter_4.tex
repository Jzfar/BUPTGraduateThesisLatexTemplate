...
数据采集采用三阶段流水线方式进行：首先从Trends24网站（trends24.in）爬取目标地区24小时内持续时间最长的前5个热门趋势话题作为采集种子，选择持续时间较长的话题旨在确保话题下已积累足够数量的评论；随后基于话题关键词在X平台检索每个话题下热度最高的前5条推文作为根推文（Root Tweet）；最后对每条根推文递归获取其完整的评论树结构，包括直接评论及多层嵌套的子评论，若某条推文本身为其他推文的评论，则回溯至最顶层根推文后进行完整采集。采集过程中同步记录trending\_zone（话题来源地区）和search\_keyword（检索关键词）等元数据字段。选择从热门趋势入手进行采集，主要考量是热门话题的时效性能够确保采集窗口覆盖Grok等AI代理功能上线后的评论数据，从而保证数据集中包含足够的AI代理样本。

原始采集数据经comment\_id去重后包含255,754条唯一评论。对其进行质量过滤，剔除crawl\_time缺失（64,631条）和comment\_text为空（8,600条）的样本，共移除73,231条（28.63\%），保留\textbf{182,523}条。评论的回复标签基于实际采集到的评论树父子关系判定（即在评论树中是否存在直接子回复），而非X平台API返回的reply\_count元数据字段，基于评论树结构判定的有回复评论为54,359条。
...
综合上述分析（如图\ref{fig:t_selection}所示），$T=12\text{h}$为帕累托最优点：正例覆盖度96.47\%，删失率仅12.90\%，且符合社交媒体研究中12--24小时短期传播窗口的领域惯例\cite{pfeffer2023halflife}。基于此选定$T=12\text{h}$后，根据4.1节提出的无右删失约束（$A \geq T$），剔除随访时间不足12小时的样本23,551条（12.90\%）。最终数据集包含\textbf{158,972}条评论，其中human 155,529条（97.83\%）、grok 3,443条（2.17\%）；整体正例率22.79\%，但两类主体的正例率差异显著：human为22.10\%，grok高达54.26\%。

此外，本数据集的覆盖范围具有明确边界。数据来源集中于X平台，AI代理主体为Grok，且human与grok样本数量存在不平衡（97.83\%/2.17\%）。这一设置与本文关注的X平台人类用户和Grok共存场景相匹配，也有助于在相对可控的主体设定下分析多主体传播差异；但实验结论仍应主要解释为该平台和该AI主体条件下的结果，其跨平台、多AI代理类型以及更均衡样本分布下的泛化能力仍需后续数据验证。
...
本章提出Group-aware Fusion TabTransformer模型，通过将主体标识作为显式特征组并引入门控融合机制，实现了评论级别的传播价值量化评估。在X平台158,972条评论的多主体数据集上，模型在整体与分主体指标上均优于四个基线；与TabTransformer、AutoInt等注意力模型的对比说明，组级建模比单纯引入通用注意力结构更适合本文的多主体场景。消融实验进一步验证了主体标识特征与门控融合机制的有效性，其中G4特征组对agent高传播价值评论识别贡献尤为明显。

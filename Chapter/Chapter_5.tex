...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{G4-2/图4-2 交互类型编码层机制图.cropped.pdf}
...
本章使用与第四章\ref{sec:dataset}节相同的X平台原始数据集（255,754条评论）重构评论树，但不对空文本节点进行过滤。本章的2跳标签由评论树结构与时间戳决定，文本缺失不影响标签有效性；同时，路径级任务中移除中间节点会导致其所有后代节点的路径需要重构，保留空文本节点以维护路径结构完整性，该节点的文本特征以零向量表示。在此基础上进行右删失过滤：移除观测截止时间（$t_{\text{obs}}$）缺失的样本（无法判断随访时长）以及随访时长不足12小时（$A < T$）的样本（$y=0$含义不明确），右删失过滤策略与第四章保持一致。
...
需要指出的是，本章数据集与第四章同源，因此同样存在平台来源和主体覆盖范围的限制。用于传播延续预测的数据中Agent节点为3,488个，占比约2.09\%，明显少于Human节点；同时，AI主体仍主要对应X平台上的Grok账号。上述不平衡分布反映了当前平台实际采样条件，也使得模型需要在Agent样本相对稀疏的情况下学习异构交互模式。实验结果能够说明本文方法在当前X平台Grok参与讨论场景下的有效性，但对于其他社交平台、其他AI代理类型以及更均衡的人机互动数据，还需要进一步开展跨平台和多主体扩展验证。

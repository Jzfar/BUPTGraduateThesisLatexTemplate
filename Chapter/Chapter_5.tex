...
第三章的系统需求分析将评论传播评估划分为消息层、路径层与交互类型层三个维度。第四章针对消息层提出了评论传播价值评估模型，预测单条评论是否会在时间窗口$T$内获得至少一次回复（1跳增长）。然而，单条评论获得回复仅代表信息引发了即时关注，而信息能否沿讨论路径持续扩散、引发多轮讨论，才是衡量传播延续能力的核心特征\cite{ziegler2019predicting}。本章进一步关注路径层与交互类型层的评估需求，聚焦于路径级别的传播延续预测——判断评论树中的节点能否在时间窗口$T$内实现2跳增长（即至少产生"回复的回复"），并显式建模路径上不同主体交互序列对传播延续的影响。在系统实现中，2跳增长概率作为路径维度的离线评估结果，用于对全量节点进行统一评分，并在按主体类型聚合后支撑多主体传播影响力对比。
...
已有的信息级联预测研究\cite{cheng2014cascades,medvedev2020modelling}主要关注级联规模（如转发总量）或评论树结构的宏观动态建模，通常将级联中的参与者视为同质主体，不区分人类用户与AI代理的行为差异，也不显式建模路径上相邻节点间的交互类型。节点级方法仅基于当前节点特征进行预测，忽略了祖先节点的上下文信息对传播延续的影响。标准的序列建模方法（如直接使用LSTM或Transformer编码路径）虽然能够捕捉节点的顺序关系，但缺乏对边级异构交互模式的显式感知，难以区分"人类回复AI"与"AI回复人类"等不同场景对传播延续的差异化影响。
...
从系统实现角度看，传播延续预测模块与第四章的传播价值评估模块共同服务于离线评估流程：系统接收已采集并完成特征抽取的评论树路径，输出路径节点的传播延续概率，用于后续排序、筛选和展示。系统当前不包含评论树流式更新、实时级联追踪或预测结果滚动刷新功能。因此，本章采用固定时间窗口下的2跳增长标签作为训练目标，并以路径构成时刻可获得的节点特征和交互类型作为模型输入；右删失约束用于保证负例标签可靠，固定窗口设置则便于与系统批处理评估流程保持一致。
...
\begin{table}[htbp]
  \centering
  \caption{路径边异构交互类型定义与传播特征假设}
  \label{tab:interaction_type}
  \renewcommand\arraystretch{1.35}
  \zihao{5}
  \begin{tabular}{>{\centering\arraybackslash}m{1.8cm} >{\centering\arraybackslash}m{3cm} >{\centering\arraybackslash}m{7cm}}
    \toprule
    交互类型 & 含义 & 传播特征假设 \\
    \midrule
    H$\to$H & 人类回复人类 & 自然讨论，传播延续取决于话题热度 \\
    H$\to$A & 人类回复AI & 可能是质疑/追问AI，易引发后续讨论 \\
    A$\to$H & AI回复人类 & AI响应后人类可能继续追问 \\
    A$\to$A & AI回复AI & 较少见，传播模式待观察 \\
    \bottomrule
  \end{tabular}
\end{table}
...
IHPE模型采用"节点编码—交互注入—序列编码—路径聚合—分类预测"的五阶段架构，以路径$P_k = [v_0, v_1, \ldots, v_k]$为输入单元，端到端预测节点$v_k$的传播延续概率。
...
模型整体结构如图\ref{fig:ihpe_arch}所示。

\vspace{0.5\baselineskip}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.70\textwidth]{G4-1/图3 交互感知的异构路径编码器结构图.cropped.pdf}
  \caption{交互感知的异构路径编码器（IHPE）整体结构}
  \label{fig:ihpe_arch}
\end{figure}
...
其中$a_i \in \{0, 1\}$为节点$v_i$的主体类型编码，$a_i=0$表示Human，$a_i=1$表示Agent；$d_{\text{type}}$为类型嵌入维度（默认取16），因此主体类型嵌入表的大小为$2 \times d_{\text{type}}$。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.86\textwidth]{G4-2/图4-2 交互类型编码层机制图.cropped.pdf}
  \caption{交互类型编码层机制示意（以路径$P = [v_0(\text{H}), v_1(\text{A}), v_2(\text{H}), v_3(\text{A})]$为例）}
...
其中$a_{i-1}$表示父节点$v_{i-1}$的主体类型，$a_i$表示当前节点$v_i$的主体类型，0表示Human，1表示Agent。因此，$(0,0)$、$(0,1)$、$(1,0)$、$(1,1)$分别对应H$\to$H、H$\to$A、A$\to$H、A$\to$A四种交互类型。
...
其中$\mathbf{x}_i \in \mathbb{R}^{d}$为主体感知节点表示，$\mathbf{e}_{\text{interact}}^{(i)} \in \mathbb{R}^{d}$为第$i$个节点入边对应的交互类型嵌入，$\mathbf{W}_{\text{fuse}} \in \mathbb{R}^{d \times d}$为融合矩阵，输出$\mathbf{x}'_i \in \mathbb{R}^{d}$为交互增强后的节点表示。对于根节点（$i=0$），由于不存在父节点，使用特殊的[ROOT]交互类型嵌入。
...
本章使用与第四章\ref{sec:dataset}节相同的某社交平台原始数据集（255,754条评论）重构评论树，但不对空文本节点进行过滤。本章的2跳标签由评论树结构与时间戳决定，文本缺失不影响标签有效性；同时，路径级任务中移除中间节点会导致其所有后代节点的路径需要重构，保留空文本节点以维护路径结构完整性，该节点的文本特征以零向量表示。在此基础上进行右删失过滤：移除观测截止时间（$t_{\text{obs}}$）缺失的样本（无法判断随访时长）以及随访时长不足12小时（$A < T$）的样本（$y=0$含义不明确），右删失过滤策略与第四章保持一致。
...
\begin{table}[htbp]
  \centering
  \caption{数据集统计与标签分布（右删失过滤后，$T=12\text{h}$）}
...
需要指出的是，本章数据集与第四章同源，因此同样存在平台来源和主体覆盖范围的限制。用于传播延续预测的数据中Agent节点为3,488个，占比约2.09\%，明显少于Human节点；同时，AI主体仍主要对应某社交平台上的AI代理A账号。上述不平衡分布反映了当前平台实际采样条件，也使得模型需要在Agent样本相对稀疏的情况下学习异构交互模式。实验结果能够说明本文方法在当前单一平台AI代理参与讨论场景下的有效性，但对于其他社交平台、其他AI代理类型以及更均衡的人机互动数据，还需要进一步开展跨平台和多主体扩展验证。
...
IHPE在所有整体指标和分主体指标上均取得最优。与Node-Only、Mean Pooling、LSTM、Transformer和Bi-LSTM等对照方法相比，IHPE的提升来自路径上下文、顺序关系和交互类型信息的逐层引入，体现出本章方法对多主体传播延续预测任务的针对性。

\textbf{（3）路径建模的必要性}

Node-Only仅使用末节点特征，无任何路径信息。与IHPE相比，AUC-PR下降34.1\%（0.2777 vs 0.4216），R@P0.8几乎为零（0.0043 vs 0.1152），几乎完全丧失高精度预测能力。这表明路径上下文是传播延续预测的必要信息——单节点特征远不足以刻画传播延续能力。该结果也说明，本章将预测对象从单个评论节点扩展到根节点至当前节点的完整路径，是区别于普通节点级分类方法的重要设计。

\textbf{（4）序列编码的价值}

Mean Pooling使用路径上所有节点但不进行序列编码，AUC-PR为0.3416。引入LSTM序列编码后提升19.7\%（0.4088）。这说明路径节点的顺序关系包含重要的传播模式信息，不能仅依赖无序聚合。对于评论树中的传播延续预测而言，祖先节点、父节点与当前节点之间具有明确的先后回复关系，顺序编码能够保留这种沿讨论链逐步展开的上下文依赖。

值得注意的是，Transformer编码器（0.3507）与Mean Pooling（0.3416）差距甚小（$+2.7\%$），远弱于LSTM的提升幅度。原因在于Transformer的多头注意力+FFN参数量较大，在短路径主导的数据分布下过拟合风险高，参数规模与数据特征不匹配，无法有效利用顺序信息。由此可见，在本文数据条件下，轻量的单向LSTM比直接采用更复杂的通用编码器更适合作为路径序列建模单元。

\textbf{（5）交互类型编码的增益}

IHPE与LSTM基线的唯一差异在于交互类型编码。IHPE在AUC-PR上提升3.1\%（0.4216 vs 0.4088），R@P0.5提升9.9\%，R@P0.8提升57.3\%。在Human和Agent子群上均取得改善。交互类型编码在高精度场景（R@P0.8）的增益尤为显著，说明准确识别交互模式对提升预测置信度具有重要作用。该结果表明，仅将路径视为节点序列仍不足以充分描述多主体传播过程，H$\to$H、H$\to$A、A$\to$H、A$\to$A等边级主体组合为传播延续预测提供了额外的结构信息。
...
交互类型编码是模型的核心组件。移除后，整体AUC-PR下降3.04\%，R@P0.5下降9.00\%，尤其是R@P0.8骤降36.4\%——高精度场景下对交互类型感知的依赖最为突出。Human子群各指标同步下降（H-PR $-3.2\%$，H-R@0.5 $-11.2\%$，H-R@0.8 $-36.5\%$），Agent子群也出现明显下降（A-PR $-2.5\%$，A-R@0.8 $-5.5\%$）。该组件的参数极少（$5 \times d = 640$个），但作用于路径中每条边，在数据充分的H$\to$H路径和稀疏的含Agent路径上均能提供有效信号。这一结果从消融角度验证了本章显式建模主体交互类型的必要性。
...
消融实验中各变体的具体配置如下。对于w/o 主体类型嵌入，完整模型的节点特征为$\mathbf{x}_i^{\text{aug}} = [\mathbf{e}_{\text{text}}; \mathbf{e}_{\text{sentiment}}; \mathbf{e}_{\text{type}}^{(i)}]$，经$\mathbf{x}_i = \mathbf{W}_{\text{proj}} \mathbf{x}_i^{\text{aug}} + \mathbf{b}_{\text{proj}}$投影，消融时去掉类型嵌入，仅使用$\mathbf{x}_i^{\text{aug}} = [\mathbf{e}_{\text{text}}; \mathbf{e}_{\text{sentiment}}]$。对于w/o 交互类型编码，完整模型中$\mathbf{x}'_i = \mathbf{x}_i + \mathbf{W}_{\text{fuse}} \mathbf{e}_{\text{interact}}^{(i)}$，消融时直接令$\mathbf{x}'_i = \mathbf{x}_i$；此配置下模型退化为LSTM基线，两者在逻辑上等价，实验结果可交叉验证。综合上述对比与消融结果，IHPE的优势主要来自对路径上下文、节点顺序和边级主体交互关系的联合建模，而不是简单替换为参数更多的通用序列编码器。

\section{本章小结}

本章提出了交互感知的异构路径编码器（IHPE），通过主体感知节点编码、交互类型编码、LSTM序列编码与均匀池化路径聚合四个组件，实现了多主体环境下路径级别的传播延续预测。实验表明IHPE在所有指标上均取得最优；与Node-Only、Mean Pooling和普通LSTM等基线的对比说明，路径上下文、顺序关系和交互类型信息分别对传播延续预测具有明确贡献。消融结果进一步表明，交互类型编码是区分本章方法与一般路径序列模型的关键组件。本章模型为系统提供了路径级传播延续预测能力，将在第六章中封装为对应的服务模块。

...
针对系统的四项核心功能，设计功能测试用例并进行验证。四项测试按"综合排序—两个评估模块独立演示—分主体评估"的逻辑组织：首先展示以传播价值与传播延续加权综合排序的评估结果列表，随后分别演示传播价值评估与传播延续预测两个模块的独立可视化功能，最后展示多主体对比分析的分主体评估结果。

\subsubsection{综合评估结果排序功能测试}

综合评估结果排序功能测试用例如表\ref{tab:test_rank}所示，排序结果效果如图\ref{fig:test_rank}所示。

\begin{table}[H]
  \centering
  \caption{综合评估结果排序功能测试用例}
  \label{tab:test_rank}
  \renewcommand\arraystretch{1.3}
  \footnotesize
  \begin{tabular}{|>{\centering\arraybackslash}m{1.5cm}|>{\raggedright\arraybackslash}m{5.2cm}|>{\raggedright\arraybackslash}m{5.8cm}|}
    \hline
    序号 & \centering 输入及操作说明 & \centering\arraybackslash 期望测试结果 \\
    \hline
    1 & 进入"评估结果"页面，选择已完成评估的数据集，观察顶部筛选栏 & 筛选栏提供"传播价值$\geq$"和"增长概率$\geq$"两个阈值输入框、主体类型下拉筛选（全部/Human/Agent）、排序方式选择（加权平均）以及传播价值与增长概率的权重比例配置（默认1:1） \\
    \hline
    2 & 将"传播价值$\geq$"设为0.4、"增长概率$\geq$"设为0.4，权重比例保持1:1，点击"查询" & 列表仅展示同时满足两个阈值条件的评论节点，按综合评分（传播价值与增长概率的加权平均）降序排列，每行包含主体类型标签、用户名、评论内容摘要、传播价值评分（绿色渐变条）、增长概率（黄色渐变条）、情感概率分布（正/中/负）及发布时间 \\
    \hline
    3 & 观察排名靠前的评论条目及其主体类型分布 & 排名第1的Human评论传播价值0.820、增长概率0.880，排名第2的Agent评论传播价值0.790、增长概率0.830，Human与Agent在高排名区域均有分布，体现系统对不同主体传播潜力的无偏评估 \\
    \hline
    4 & 对比排名靠前与靠后的条目，点击靠前条目的"树图"按钮 & 排名靠前的评论两项评分均较高，靠后的评论至少一项接近阈值下限；点击"树图"按钮后跳转至该评论所在评论树的可视化视图，实现从列表到图表的联动 \\
    \hline
    测试结果 & \multicolumn{2}{l|}{通过} \\
    \hline
    异\quad 常 & \multicolumn{2}{l|}{无} \\
    \hline
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{TEST/综合评估结果排序功能测试.png}
  \caption{综合评估结果排序效果图}
  \label{fig:test_rank}
\end{figure}
...
评论传播价值评估功能测试用例如表\ref{tab:test_value}所示，评估结果的树形可视化效果如图\ref{fig:test_value}所示。

\begin{table}[H]
  \centering
  \caption{评论传播价值评估功能测试用例}
  ...
  \begin{tabular}{|>{\centering\arraybackslash}m{1.5cm}|>{\raggedright\arraybackslash}m{5.2cm}|>{\raggedright\arraybackslash}m{5.8cm}|}
    ...
    序号 & \centering 输入及操作说明 & \centering\arraybackslash 期望测试结果 \\
    \hline
    1 & 进入"传播价值"页面，选择已评估数据集，在根节点下拉框中选择一棵评论树，点击"重绘" & 页面渲染树形结构，节点颜色区分主体类型（蓝色为Human、橙色为Agent），节点大小与传播价值评分成正比，底部图例标注数据集名称、节点统计（含Human/Agent数量）及"节点大小 = 传播价值评分" \\
    \hline
    2 & 观察同一父节点下的多个子节点的大小差异 & 有后续回复的子节点圆形明显大于无回复的兄弟节点，体现模型在相同上下文下能够区分不同评论的传播价值差异 \\
    \hline
    3 & 将鼠标悬停在一个圆形极小的节点上，查看浮动提示信息 & 提示框显示用户名、主体类型（Human）、传播价值评分（0.0600）、增长概率（0.0400），评论内容为单个表情符号，信息量低且未获得回复，与其低传播价值评分一致 \\
    \hline
    4 & 对比树中不同位置的大节点与小节点 & 大节点（高传播价值）多出现在讨论活跃的分支上，小节点（低传播价值）多为讨论终止的叶子节点，评分分布符合评论树的实际传播结构 \\
    \hline
    测试结果 & \multicolumn{2}{l|}{通过} \\
    \hline
    异\quad 常 & \multicolumn{2}{l|}{无} \\
...
传播延续预测功能测试用例如表\ref{tab:test_growth}所示，预测结果的树形可视化效果如图\ref{fig:test_growth}所示。

\begin{table}[H]
  \centering
  \caption{传播延续预测功能测试用例}
  ...
  \begin{tabular}{|>{\centering\arraybackslash}m{1.5cm}|>{\raggedright\arraybackslash}m{5.2cm}|>{\raggedright\arraybackslash}m{5.8cm}|}
    ...
    序号 & \centering 输入及操作说明 & \centering\arraybackslash 期望测试结果 \\
    \hline
    1 & 切换至"传播延续"页面，选择同一棵评论树，点击"重绘" & 页面渲染树形结构，连边颜色与粗细反映2跳增长概率：红色粗线表示高增长概率、橙色中等线表示中等概率、浅灰细线表示低概率 \\
    \hline
    2 & 观察根节点的出边颜色与粗细 & 根节点到多个一级回复的边多为红色粗线，表明这些路径的起始段具有较高的传播延续概率 \\
    \hline
    3 & 沿讨论链追踪连边颜色的变化趋势 & 部分路径从红色逐步过渡到橙色再到灰色，反映传播延续能力随路径深度递减；另有路径持续保持红色或橙色延伸至较深层级，表示讨论链具有较强的延续动能 \\
    \hline
    4 & 对比不同分支的边颜色模式 & 短分支以灰色细线为主，讨论在浅层终止；长分支包含连续的红色或橙色边，讨论延续至较深层级，体现模型对不同讨论链传播延续能力的区分 \\
    \hline
    测试结果 & \multicolumn{2}{l|}{通过} \\
    \hline
    异\quad 常 & \multicolumn{2}{l|}{无} \\
...
本章在第三章系统设计的基础上，阐述了多主体信息传播影响力评估系统六个功能模块从数据预处理、存储、模型推理到对比分析与可视化展示的具体实现方案。功能测试与性能测试结果表明，系统端到端流程正确，各环节响应效率满足交互式分析的使用需求。
